...
namespace Chat_Application.SocketsManger
{
    public abstract class SocketHandler
    {
        ...
        {
            await Task.Run(() => { Connections.GetID(socket); });
        ...
        public  async Task SendMessage(WebSocket socket,string message)
        {
            ...
            await socket.SendAsync(new System.ArraySegment<byte>(Encoding.ASCII.GetBytes(message), 0, message.Length), WebSocketMessageType.Text, true, CancellationToken.None);
